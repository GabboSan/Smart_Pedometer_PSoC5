...
void Clock_1ms_Start(void) 
{
    ...
    Clock_1ms_CLKEN |= Clock_1ms_CLKEN_MASK;
	Clock_1ms_CLKSTBY |= Clock_1ms_CLKSTBY_MASK;
}
...
void Clock_1ms_Stop(void) 
{
    ...
    Clock_1ms_CLKEN &= (uint8)(~Clock_1ms_CLKEN_MASK);
	Clock_1ms_CLKSTBY &= (uint8)(~Clock_1ms_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_1ms_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_1ms_StopBlock(void) 
{
    if ((Clock_1ms_CLKEN & Clock_1ms_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_1ms__CFG3)
        CLK_DIST_AMASK = Clock_1ms_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_1ms_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_1ms__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_1ms_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_1ms_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_1ms_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_1ms_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_1ms_CLKSTBY &= (uint8)(~Clock_1ms_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_1ms_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_1ms_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_1ms_GetSourceRegister();
    uint16 oldDivider = Clock_1ms_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_1ms_CLKEN & Clock_1ms_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_1ms_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Clock_1ms_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_1ms_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_1ms_SetModeRegister(uint8 modeBitMask) 
{
    Clock_1ms_MOD_SRC |= modeBitMask & (uint8)Clock_1ms_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_1ms_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_1ms_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_1ms_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_1ms_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_1ms_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_1ms_GetModeRegister(void) 
{
    return Clock_1ms_MOD_SRC & (uint8)(Clock_1ms_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_1ms_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_1ms_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_1ms_GetDividerRegister();
    uint8 oldSrc = Clock_1ms_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_1ms_MOD_SRC |= CYCLK_SSS;
        Clock_1ms_MOD_SRC =
            (Clock_1ms_MOD_SRC & (uint8)(~Clock_1ms_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_1ms_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_1ms_GetSourceRegister(void) 
{
    return Clock_1ms_MOD_SRC & Clock_1ms_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_1ms_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_1ms_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_1ms_PHASE = clkPhase & Clock_1ms_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_1ms_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_1ms_GetPhaseRegister(void) 
{
    return Clock_1ms_PHASE & Clock_1ms_PHASE_MASK;
}
