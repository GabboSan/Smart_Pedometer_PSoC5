...
// for madgwick
const float GyroMeasError = PI_ * (40.0f / 180.0f);     // gyroscope measurement error in rads/s (start at 40 deg/s)
const float GyroMeasDrift = PI_ * (0.0f / 180.0f);      // gyroscope measurement drift in rad/s/s (start at 0.0 deg/s/s)
const float beta = 0.866f * PI_ * (40.0f / 180.0f);  // compute beta
const float zeta = 0.866f * PI_ * (0.0f / 180.0f);  // compute zeta, the other free parameter in the Madgwick scheme usually set to a small or zero value
double deltaT = 0.01; //10 ms is time of the ISR 
